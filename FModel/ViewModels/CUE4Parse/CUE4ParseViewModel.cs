...
public partial class CUE4ParseViewModel : ViewModel
{
    ...
    public CUE4ParseViewModel()
    {
        ...
        //        var secondVersionContainer = new VersionContainer(
        //            game: EGame.GAME_UE4_27, platform: currentDir.TexturePlatform,
        //            customVersions: new FCustomVersionContainer(currentDir.Versioning.CustomVersions),
        //            optionOverrides: currentDir.Versioning.Options,
        //            mapStructTypesOverrides: currentDir.Versioning.MapStructTypes);

        //        var secondGameDirectory = @"D:\depotdownloader-2.4.7\depots\381211\11239367\DeadByDaylight\Content\Paks";
        //        DiffProvider = new DefaultFileProvider(secondGameDirectory, SearchOption.AllDirectories, secondVersionContainer,
        //            pathComparer);
    ...
    //            Log.Information($"Second Provider: {DiffProvider.Versions.Game} | Archives: x{DiffProvider.UnloadedVfs.Count} | AES: x{Provider.RequiredKeys.Count} | Loose Files: x{DiffProvider.Files.Count}");
    //        }
    ...
    //        DiffProvider?.TryChangeCulture(Provider.GetLanguageCode(UserSettings.Default.AssetLanguage));
    //    });
    //}
    ...
    public string ExtractTextForDiff(AbstractVfsFileProvider provider, GameFile entry)
    {
        ...
        switch (ext)
        {
            case "uasset":
            {
                ...
                {
                    return $"[Failed to load uasset: {ex.Message}]";
                }
            }
            default:
                try
    ...
    public (string left, string right) GetExtractedTextsForDiff(string assetPath)
    {
        ...
        string left = entry1 != null ? ExtractTextForDiff(DiffProvider, entry1) : "[Missing]";
        string right = entry2 != null ? ExtractTextForDiff(Provider, entry2) : "[Missing]";
    ...
    public void ShowAssetDiff(string assetPath)
    {
        ...
        {
            var diffViewer = new JsonDiffViewer(left, right);
